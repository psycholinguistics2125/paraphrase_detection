...
def insert_paraphrase(original_text, paraphrase):
    return original_text.replace(paraphrase, f"<span style='background-color: yellow;'>{paraphrase}</span>")
...
def main():
    ...
    dataset_folder = st.sidebar.selectbox("Select Dataset", [x[0] for x in os.walk(altered_data_folder)][1:])
    ...
    if dataset_folder :
        ...
        config_dataset = load_config(os.path.join(dataset_folder, "dataset_model_kwargs.yaml"))
    
     
        st.sidebar.write("Dataset Summary:")
        st.sidebar.write(f"Number of Examples: {len(dataset)}")
        st.sidebar.write(f"The paraphrases were generated using the model: {config_dataset['model_name']}")
    
        if "ibm" in config_dataset['model_name']: 
            str_param = [f"{k} : {v}" for k,v in config_dataset['quality_control_kwargs'].items()]
        else : 
            str_param = [f"{k} : {v}" for k,v in config_dataset['parrot_kwargs'].items()]
        ...
        st.subheader(f"The sentence n°{int(para_index + 1)} was paraphrased into n°{int(para_index + 2)}")
        ...
        modified_sentence = st.text_area(f"Original sentence:  {sentences[para_index]}", sentences[para_index+1], key=f"sentence-{para_index}",height=100)
