...
class ParaphraseGenerator:
    def __init__(self, model_name, logger=None, parrot_kwargs=None, quality_control_kwargs=None):
    ...
    def load_generator(self):
        if self.model_name in ["Vamsi/T5_Paraphrase_Paws", 'humarin/chatgpt_paraphraser_on_T5_base',
                               "prithivida/parrot_paraphraser_on_T5",
                               "stanford-oval/paraphraser-bart-large"]:
            self.generator = ParrotGenerator(model_name = self.model_name,kwargs=self.parrot_kwargs)
            self.logger.info("Loaded ParrotGenerator")
        elif self.model_name in ["ibm/qcpg-sentences", "ibm/qcpg-captions", "ibm/qcpg-questions"]:
            self.generator = QualityControlGenerator(model_name = self.model_name, kwargs = self.quality_control_kwargs)
            self.logger.info("Loaded QualityControlGenerator")
        else:
            self.logger.error("Invalid model_name. Supported values: Parrot or Quality Control models.")
            raise ValueError("Invalid model_name. Supported values: Parrot or Quality Control models.")

    def paraphrase(self, input_phrase):
        if self.generator is not None:
            self.logger.info("Generating paraphrase using {} for input: {}".format(self.model_name, input_phrase))
            return self.generator.paraphrase(input_phrase)
        else:
            self.logger.error("Generator has not been loaded. Call load_generator first.")
            raise ValueError("Generator has not been loaded. Call load_generator first.")
...
def introduce_paraphrase(text, index, paraphrase_generator):
    ...
    if index < 0 or index >= len(sentences):
        raise ValueError("Invalid value of I. It should be within the range [0, {}].".format(len(sentences) - 1))
...
def introduce_paraphrases_to_dataset(df, model_name, save_path=None, logger=None):
    ...
    for index, row in df.iterrows():
        ...
        try:
            ...
            modified_text = introduce_paraphrase(index = index_paraphrase, paraphrase_generator=generator, text = text)
