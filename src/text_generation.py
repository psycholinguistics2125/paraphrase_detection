...
def load_model(model_name = "gpt2", logger = logging.getLogger(__name__)):
    ...
    if model_name == "falcon":
        ...
        model = AutoModelForCausalLM.from_pretrained("gpt2-large",pad_token_id=tokenizer.eos_token_id).to(device)
...
def generate_from_prompt(prompt, model, tokenizer, 
                        do_sample =True,
                        prospective_span=5,
    ...
    input_ids_temp=input_ids[:,max(0,input_ids.shape[1]-retrospective_span):input_ids.shape[1]]
    output_all=input_ids 
    cont = True

    while cont:
        temp_output = model.generate(input_ids_temp, 
                                    do_sample=do_sample,
        ...
        input_ids_temp=output_all[:,max(0,output_all.shape[1]-retrospective_span):output_all.shape[1]]

        if output_all.shape[1]>=target_length: cont=False
        
        input_length=retrospective_span
        nPunct=sum(sum(output_all[0,-input_length:]==i for i in punc_tokens).bool())
...
def generate_dataset_from_config(config, logger = logging.getLogger(__name__), save = False, clean = False):
    ...
    logger.info(f"Temperature range : {config['min_temperature']} - {config['max_temperature']}")

    # Init dataset
    dataset = pd.DataFrame(columns = ["prompt", "generated_text", "model_name", "temperature", "num_beams"])

    temperature_list = generate_random_temperature(n_sim = n_sim, min = config['min_temperature'], max = config['max_temperature'])
    ...
    for prompt in prompt_list:
        ...
        for i in tqdm(range(config['n_simulations'])):
            ...
            dataset.loc[k] = pd.Series({"prompt" : prompt,
    ...
    if clean :
        ...
        dataset["text"] = dataset["generated_text"].apply(lambda x : clean_generated_text(x))
