...
if __name__ == "__main__": 
    ...
    # generate the index
    dataset["index_paraphrase"] = dataset['text'].apply(lambda x : random.randint(1, len(x.split("."))-1))
    # Apply model
    model_name = config["model_name"]
    logger.info("Loading model %s", model_name)
    saving_folder = os.path.join(config["saving_folder"], f"{model_name.replace('/','-')}_{dataset_name}")
    if not os.path.exists(saving_folder):
        os.makedirs(saving_folder)
    saving_path = os.path.join(saving_folder, f"altered_dataset.csv")

    altered = introduce_paraphrases_to_dataset(dataset, model_name, logger=logger, save_path = saving_path)
    ...
    logger.info("Model kwargs saved to %s", os.path.join(saving_folder, "dataset_model_kwargs.csv"))
